...
class Employee:
  ...
  def give_raise(self, raise_amount):
      ...
      print(f"Amount {raise_amount} is added into {self.name}'s existing salary, and your updated salary is :: {self.salary} which was {initial_salary} before the raise.")
      
      
if __name__ == "__main__":
    
 employee_one = Employee("Dipak Neupane", 2000, "Senior Software Engineer")
 print(f"{employee_one.name.split()[0]}'s employment details are :: {employee_one.display_info()}")
 
 # Raise first employee's salary by 20.
 employee_one.give_raise(20)
 print(f"After the raise, {employee_one.name.split()[0]}'s employment details are :: {employee_one.display_info()}")
 
 employee_two = Employee("Bhuwan Bhatt", 3000, "Senior Cloud Architect")
 print(f"{employee_two.name.split()[0]}'s employment details are :: {employee_two.display_info()}")
 
 # Raise second employee's salary by 30.
 employee_two.give_raise(30)
 print(f"After the raise, {employee_two.name.split()[0]}'s employment details are :: {employee_two.display_info()}")
