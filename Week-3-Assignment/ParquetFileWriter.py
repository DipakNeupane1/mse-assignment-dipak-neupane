...
class ParquetFileWriter:
     ...
     def csv_file_reader(self, file_path):
      ...
      df.to_parquet("Week-3-Assignment/iris/iris.parquet", engine="pyarrow", index=False)
      
      # Reading parquet file from a directory.
      parquet_df = pd.read_parquet("Week-3-Assignment/iris/iris.parquet", engine="pyarrow")
      return parquet_df
     ...
     def compute_min_max_and_avg_value(self, parquet_df):
         ...
         print("Datatypes after selecting only numeric columns \n " , numeric_dataframe.dtypes)
